...
if(isset($_GET['act'])){
	...
	if($_GET['act']=='insert'){
		...
		//menyimpankiriman form ke variabel
		$nip		 = $_POST['nip'];
		...
		if ($nip=='' || $nama=='' || $jab=='' || $gol=='' || $status=='' || $anak==''){
			...
			$simpan = mysqli_query($konek, "INSERT INTO pegawai(nip,nama_pegawai,kode_jabatan,kode_golongan,status,jumlah_anak)
				VALUES('$nip','$nama','$jab','$gol','$status','$anak')");
	...
	elseif($_GET['act']=='update'){
		...
		if ($nip=='' || $nama=='' || $jab=='' || $gol=='' || $status=='' || $anak==''){
			...
			$update = mysqli_query($konek, "UPDATE pegawai SET nama_pegawai='$nama',
									kode_jabatan='$jab',
									kode_golongan='$gol',
									status='$status',
									jumlah_anak='$anak'
